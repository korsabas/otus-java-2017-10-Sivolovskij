...
public class MessageProcess 
{
    ...
    public static void main( String[] args ) throws Exception
    {
    	...
    	int webPort = -1;
    	if (args.length == 0) {
    		ProcessBuilder pb = new ProcessBuilder("java", "-jar", "socketchat.jar", "-m", "messageService", "-h", "localhost", "-p", "8181");
    		Process process = pb.start();
    		System.out.println("Starting message");
    		pb = new ProcessBuilder("java", "-jar", "socketchat.jar", "-m", "dbservice", "-h", "localhost", "-p", "8181");
    		process = pb.start();
    		System.out.println("Starting dbservice");
    		pb = new ProcessBuilder("java", "-jar", "socketchat.jar", "-m", "dbservice", "-h", "localhost", "-p", "8181");
    		process = pb.start();
    		System.out.println("Starting dbservice");
    		pb = new ProcessBuilder("java", "-jar", "socketchat.jar", "-m", "webfront", "-h", "localhost", "-p", "8181", "-wp", "8081");
    		process = pb.start();
    		System.out.println("Starting webfront");
    		pb = new ProcessBuilder("java", "-jar", "socketchat.jar", "-m", "webfront", "-h", "localhost", "-p", "8181", "-wp", "8082");
    		process = pb.start();
    		System.out.println("Starting webfront");
